...
function push_to_queue_recharge(
  variantID,
  quantity,
  frequency,
  unit_type,
...
ShopifyAPI.addItemToCart = function (
  variant_id,
  qty,
  frequency,
  unit_type,
...
var ajaxCart = (function (module, $) {
  ...
  init = function (options) {
    ...
    settings = {
      ...
      discountedPriceCurrency: ".discounted-price-currency",
    ...
    $discountedPriceCurrency = $(settings.discountedPriceCurrency);
  ...
  returnSinglePrice = function (type) {
    ...
    //console.log("returnsingleprice");
    if ($price.data("price-recharge") == "true") {
      //console.log("recharge-price");
      price = ShopifyAPI.formatMoney(
        qty * $price.attr("data-rc-price"),
        $price.html().charAt(0)
      ).substr(1);
      priceCompare = qty * $price.attr("data-rc-price");
    } else {
      //console.log("onetime-price");
      price = ShopifyAPI.formatMoney(
        qty * $price.attr("data-onetime-price"),
        $price.html().charAt(0)
      ).substr(1);
      priceCompare = qty * $price.attr("data-onetime-price");
    }

    if (type == "discounted") {
      if (
        $discountedPrice.attr("data-compare-price") &&
        $discountedPrice.attr("data-compare-price") != "0"
      ) {
        //console.log('compare-price');
        discountedPrice = ShopifyAPI.formatMoney(
          qty * $discountedPrice.attr("data-compare-price"),
          $price.html().charAt(0)
        ).substr(1);
        discountedPriceCompare =
          qty * $discountedPrice.attr("data-compare-price");
      } else {
        //console.log('custom-compare-price');
        discountedPrice = ShopifyAPI.formatMoney(
          qty * $discountedPrice.attr("data-compare-custom-price"),
          $price.html().charAt(0)
        ).substr(1);
        discountedPriceCompare =
          qty * $discountedPrice.attr("data-compare-custom-price");
      ...
      //console.log(discountedPriceCompare  + " vs " + priceCompare);
      if (discountedPriceCompare > priceCompare) {
        //console.log('adding that class');
        $discountedPrice.removeClass("hide");
        $discountedPriceCurrency.removeClass("hide");
      } else {
        //console.log('removing that class');
        $discountedPrice.addClass("hide");
        $discountedPriceCurrency.addClass("hide");
      }
  ...
  formOverride = function () {
    $form.on("submit", function (evt) {
      ...
      $addToCartMultiple.removeClass("is-added").addClass("is-adding");
  ...
  addCustomItems = function () {
    ...
    $multipleForm.on("submit", function (evt) {
      ...
      if (
        $(this).find('input[name="purchase-type"][value="Recurring"]:checked')
          .length > 0
      ) {
        //console.log('recurring');
        $addToCartType = "recurring";
        var frequency = $(this)
          .find("select#shipping_interval_frequency")
          .val();
      } else {
        $addToCartType = "one-time";
      }
    ...
    $purchaseType.on("change", function () {
      ...
      switch ($(this).val()) {
        case "Recurring":
          ...
          $price.html(returnSinglePrice());
          $discountedPrice
            .html(returnSinglePrice("discounted"))
            .removeClass("hide");
          $addToCartMultiple.find("[data-add-to-cart-texts]").text("Subscribe");
        ...
        case "One Time":
          ...
          $('input[name="purchase-type"][value="Recurring"]').prop(
            "checked",
            false
          );
  ...
  itemAddedCallback = function (product) {
    $addToCartMultiple.removeClass("is-adding").addClass("is-added");
  ...
  buildCart = function (cart) {
    ...
    if (width >= 320 && width <= 767) {
      ...
      var cookieCurrency = jQuery(".mobile-header-message-currency")
        .find(".currency-picker")
        .val();
      // Currency.convertAll(defaultCurrency, cookieCurrency, 'span.money');
    } else {
      var defaultCurrency = "GBP";
      var cookieCurrency = jQuery("#announcement-bar")
        .find(".currency-picker")
        .val();
      //Currency.convertAll(defaultCurrency, cookieCurrency, 'span.money');
    }
